...
from videox_fun.data.dataset_image_video import (ImageVideoDataset,
                                                 MovieBenchFirstFramePairDatasetSeg,
                                                 ImageVideoSampler,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if (args.validation_prompts is not None or args.validation_data_jsonl is not None) and global_step % args.validation_steps == 0:
                        log_validation(
                            vae,
                            text_encoder,
                            tokenizer,
        ...
        if accelerator.is_main_process:
            if (args.validation_prompts is not None or args.validation_data_jsonl is not None) and epoch % args.validation_epochs == 0:
                log_validation(
                    vae,
                    text_encoder,
                    tokenizer,
...
if __name__ == "__main__":
    # import debugpy
    # debugpy.listen(("127.0.0.1", 5674))  # 只监听本机，最安全
    # print("debugpy is listening on 127.0.0.1:5679")
