...
from videox_fun.utils.lora_utils import (create_network, merge_lora,
                                         unmerge_lora)
from videox_fun.utils.utils import get_image_to_video_latent, get_video_to_video_latent, save_videos_grid
...
def log_validation(vae, text_encoder, tokenizer, transformer3d, network, config, args, accelerator, weight_dtype, global_step):
    try:
        ...
        # SEP token for prompt concatenation
        SEP = "ad23r2 the screen suddenly turns white, then the camera view suddenly changes."
        
        # Helper function to sample video frames as tensor
        def _sample_video_frames_as_tensor_v2(video_path, num_frames, height, width, sampling="contiguous"):
            ...
            video = torch.from_numpy(frames).permute(3, 0, 1, 2).unsqueeze(0).contiguous()
            video = video.to(dtype=torch.float32) / 255.0
            return video

        # Helper function to construct input_video and mask from source video
        def _get_video_to_video_latent_for_val(source_video_path, video_length, sample_size, src_frame_sampling="contiguous"):
        ...
        # Load validation data from JSONL if provided
        validation_items = []
        if args.validation_data_jsonl and os.path.exists(args.validation_data_jsonl):
            import json
            with open(args.validation_data_jsonl, "r", encoding="utf-8") as f:
                for line in f:
                    line = line.strip()
                    if not line:
                        continue
                    validation_items.append(json.loads(line))
        
        # Use JSONL validation if available and train_mode is not normal
        if validation_items and args.train_mode != "normal":
            item = random.choice(validation_items)
            source_path = item.get("source_abs")
            if source_path and os.path.exists(source_path):
                # Construct prompt from refined_source_prompt and edited_video_prompt
                ...
                with torch.no_grad():
                    with torch.autocast("cuda", dtype=weight_dtype):
                        ...
                        input_video, input_video_mask = _get_video_to_video_latent_for_val(
                            source_path,
                            video_length=video_length,
                            sample_size=sample_size,
                            src_frame_sampling="contiguous",
                        )
                        input_video = input_video.to(accelerator.device)
                        input_video_mask = input_video_mask.to(accelerator.device)
                        ...
                        save_videos_grid(sample, os.path.join(args.output_dir, f"sample/sample-{global_step}-val.gif"))
        elif args.validation_prompts:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="robot",
        choices=("auto", "character_pair", "robot"),
    )
    parser.add_argument(
        "--video_root",
        type=str,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If set, use a white frame between source and edited videos for transition.",
    )
    # Robot dataset arguments
    parser.add_argument(
        "--robot_data_root",
        type=str,
        default="/gemini/code/datasets/libero/perturbed",
        help="Path to LIBERO perturbed dataset root",
    )
    parser.add_argument(
        "--robot_source_data_root",
        type=str,
        default="/gemini/code/datasets/libero/datasets",
        help="Path to LIBERO original dataset root for instruction extraction",
    )
    parser.add_argument(
        "--robot_suites",
        nargs="+",
        default=None,
        help="List of suites to include, e.g., libero_10 libero_90",
    )
    parser.add_argument(
        "--libero_data_root",
        type=str,
        default=None,
        help="LIBERO HDF5 dataset root. If set, it overrides --robot_data_root for dataset_type=robot.",
    )
    parser.add_argument(
        "--libero_first_frame_prob",
        type=float,
        default=0.1,
        help="Probability to sample a segment starting from the first frame in LiberoDataset.",
    )
    parser.add_argument(
        "--max_train_samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to validation JSONL file (e.g., moviebench_firstframe_pair_test.jsonl) for MovieBench validation.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def main():
    ...
    def _iter_robot_extra_modules(model):
        for _name in ["action_embedding", "proprio_embedding", "action_head", "propriohead"]:
            if hasattr(model, _name):
                yield _name, getattr(model, _name)

    def _enable_robot_extra_trainable(model):
        for _, _mod in _iter_robot_extra_modules(model):
            _mod.requires_grad_(True)

    def _zero_init_robot_extra_modules(model):
        with torch.no_grad():
            for _name, _mod in _iter_robot_extra_modules(model):
                # 跳过 embedding 模块，它们已有正确的初始化（gate=1, last layer zero init）
                if _name in ["action_embedding", "proprio_embedding"]:
                    continue
                for _p in _mod.parameters(recurse=True):
                    _p.zero_()

    def _get_robot_extra_state_dict(model):
        extra_state = {}
        for _name, _mod in _iter_robot_extra_modules(model):
            for k, v in _mod.state_dict().items():
                extra_state[f"{_name}.{k}"] = v
        return extra_state

    def _load_robot_extra_state_dict(model, state_dict):
        for _name, _mod in _iter_robot_extra_modules(model):
            prefix = f"{_name}."
            sub = {k[len(prefix):]: v for k, v in state_dict.items() if k.startswith(prefix)}
            if len(sub) > 0:
                _mod.load_state_dict(sub, strict=True)

    def _log_robot_embed_sanity(model, step, writer=None):
        """Log sanity check for ActionEmbed/ProprioEmbed to verify training is working."""
        for embed_name in ["action_embedding", "proprio_embedding"]:
            if not hasattr(model, embed_name):
                continue
            embed = getattr(model, embed_name)
            
            # Gate value
            gate_val = embed.gate.data.item()
            
            # Gradient info
            gate_grad = embed.gate.grad
            gate_grad_norm = gate_grad.norm().item() if gate_grad is not None else 0.0
            
            # MLP first layer stats (should have non-zero values)
            mlp0_weight = embed.mlp[0].weight.data
            mlp0_mean = mlp0_weight.mean().item()
            mlp0_std = mlp0_weight.std().item()
            mlp0_grad = embed.mlp[0].weight.grad
            mlp0_grad_norm = mlp0_grad.norm().item() if mlp0_grad is not None else 0.0
            
            # MLP last layer stats (initialized to zero, should start learning)
            mlp_last_weight = embed.mlp[-1].weight.data
            mlp_last_mean = mlp_last_weight.abs().mean().item()
            mlp_last_grad = embed.mlp[-1].weight.grad
            mlp_last_grad_norm = mlp_last_grad.norm().item() if mlp_last_grad is not None else 0.0
            
            prefix = "action_embed" if "action" in embed_name else "proprio_embed"
            print(f"[Sanity] step={step} {prefix}: gate={gate_val:.4f} gate_grad={gate_grad_norm:.2e} | "
                  f"mlp0: mean={mlp0_mean:.4f} std={mlp0_std:.4f} grad={mlp0_grad_norm:.2e} | "
                  f"mlp_last: |mean|={mlp_last_mean:.2e} grad={mlp_last_grad_norm:.2e}")
            
            if writer is not None:
                writer.add_scalar(f"sanity/{prefix}_gate", gate_val, step)
                writer.add_scalar(f"sanity/{prefix}_gate_grad", gate_grad_norm, step)
                writer.add_scalar(f"sanity/{prefix}_mlp0_grad", mlp0_grad_norm, step)
                writer.add_scalar(f"sanity/{prefix}_mlp_last_grad", mlp_last_grad_norm, step)
                writer.add_scalar(f"sanity/{prefix}_mlp_last_abs_mean", mlp_last_mean, step)
    ...
    _zero_init_robot_extra_modules(transformer3d)
    _enable_robot_extra_trainable(transformer3d)
                ...
                if accelerator.is_main_process:
                    ...
                    for _m in models:
                        if hasattr(_m, "action_embedding") or hasattr(_m, "action_head"):
                            robot_model = accelerator.unwrap_model(_m)
                            break
                    if robot_model is not None:
                        robot_sd = _get_robot_extra_state_dict(robot_model)
                        save_file(robot_sd, os.path.join(output_dir, "robot_extra_modules.safetensors"), metadata={"format": "pt"})
    ...
    if args.use_peft_lora:
        ...
        extra_params = []
        for _, _mod in _iter_robot_extra_modules(transformer3d):
            extra_params.extend(list(_mod.parameters()))
        extra_params = list(filter(lambda p: p.requires_grad, extra_params))
        if len(extra_params) > 0:
            trainable_params.extend(extra_params)
            if isinstance(trainable_params_optim, list) and len(trainable_params_optim) > 0 and isinstance(trainable_params_optim[0], dict):
                trainable_params_optim.append({"params": extra_params, "lr": args.learning_rate})
            else:
                trainable_params_optim = list(trainable_params_optim) + extra_params
    ...
    # Get the dataset
    if str(args.dataset_type).lower() == "robot":
        robot_suites = (
            args.robot_suites
            if args.robot_suites is not None
            else ["libero_90", "libero_10", "libero_goal", "libero_object", "libero_spatial"]
        )

        libero_root = args.libero_data_root if isinstance(args.libero_data_root, str) and args.libero_data_root else args.robot_data_root
        train_dataset = LiberoDataset(
            ...
            suites=robot_suites,
            first_frame_prob=args.libero_first_frame_prob,
        )

        if len(train_dataset) <= 0:
            raise ValueError(
                "LiberoDataset is empty. Check --libero_data_root and --robot_suites. "
                f"libero_root={libero_root}, robot_suites={robot_suites}"
            )
    elif str(args.dataset_type).lower() == "character_pair":
        if not isinstance(args.train_data_meta, str) or not args.train_data_meta:
            raise ValueError("--train_data_meta (path to dataset.csv) is required when --dataset_type=character_pair")

        video_root = args.video_root
        if not isinstance(video_root, str) or not video_root:
            video_root = args.train_data_dir

        from videox_fun.data.dataset_image_video import MovieBenchFirstFramePairDatasetSeg
        train_dataset = MovieBenchFirstFramePairDatasetSeg(
            ...
            enable_bucket=args.enable_bucket,
            sep=args.pair_sep,
            use_white_frame=args.use_white_frame,
        )
    else:
    ...
    if args.enable_bucket:
        ...
        # Use video_root for character_pair dataset, otherwise use train_data_dir
        if str(args.dataset_type).lower() == "character_pair":
            bucket_train_folder = args.video_root if args.video_root else args.train_data_dir
        elif str(args.dataset_type).lower() == "robot":
            bucket_train_folder = None
        else:
            bucket_train_folder = args.train_data_dir

        dataset_for_sampler = train_dataset.dataset
        if str(args.dataset_type).lower() == "robot":
            dataset_for_sampler = train_dataset

        batch_sampler = AspectRatioBatchImageVideoSampler(
            sampler=RandomSampler(train_dataset, generator=batch_sampler_generator), dataset=dataset_for_sampler, 
            batch_size=args.train_batch_size, train_folder = bucket_train_folder, drop_last=True,
        ...
        def collate_fn(examples):
            ...
            spatial_token_multiplier = int(examples[0].get("spatial_token_multiplier", 1))
            ...
            new_examples["spatial_token_multiplier"] = spatial_token_multiplier
            new_examples["pixel_values"] = []
            new_examples["text"]         = []
            if "actions" in examples[0]:
                new_examples["actions"] = []
            if "proprio" in examples[0]:
                new_examples["proprio"] = []
            ...
            for example in examples:
                ...
                if "actions" in new_examples:
                    actions = torch.from_numpy(example["actions"]) if isinstance(example["actions"], np.ndarray) else torch.as_tensor(example["actions"])
                    new_examples["actions"].append(actions[:batch_video_length])
                if "proprio" in new_examples:
                    proprio = torch.from_numpy(example["proprio"]) if isinstance(example["proprio"], np.ndarray) else torch.as_tensor(example["proprio"])
                    new_examples["proprio"].append(proprio[:batch_video_length])

                if args.train_mode != "normal":
                    mask = get_random_mask_for_robot(new_examples["pixel_values"][-1].size())
            ...
            if "actions" in new_examples:
                new_examples["actions"] = torch.stack([example for example in new_examples["actions"]])
            if "proprio" in new_examples:
                new_examples["proprio"] = torch.stack([example for example in new_examples["proprio"]])
            if args.train_mode != "normal":
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            dirs = [d for d in dirs if len(d.split("-")) >= 2 and d.split("-")[1].isdigit()]
        ...
        if path is None:
            ...
            if args.save_state:
            ...
            elif zero_stage != 3 and not args.use_fsdp:
                from safetensors.torch import load_file
                ...
                robot_path = os.path.join(checkpoint_folder_path, "robot_extra_modules.safetensors")
                if os.path.exists(robot_path):
                    robot_sd = load_file(robot_path, device=str(accelerator.device))
                    _load_robot_extra_state_dict(accelerator.unwrap_model(transformer3d), robot_sd)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        train_proprio_loss = 0.0
        action_loss_seen = False
        proprio_loss_seen = False
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer3d):
                ...
                spatial_token_multiplier = int(batch.get("spatial_token_multiplier", 1))

                # Increase the batch size when the length of the latent sequence of the current sample is small
                if args.auto_tile_batch_size and args.training_with_video_token_length and zero_stage != 3:
                    if args.video_sample_n_frames * args.token_sample_size * args.token_sample_size * spatial_token_multiplier // 16 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                    ...
                    elif args.video_sample_n_frames * args.token_sample_size * args.token_sample_size * spatial_token_multiplier // 4 >= pixel_values.size()[1] * pixel_values.size()[3] * pixel_values.size()[4]:
                ...
                if args.keep_all_node_same_token_length:
                    if args.token_sample_size > 256:
                        ...
                        numbers_list = [256]
                    numbers_list = [_number * _number * args.video_sample_n_frames * spatial_token_multiplier for _number in  numbers_list]
                ...
                if spatial_compression_ratio >= 16:
                    ...
                    mask_conditions[:, :, 1:, :, :] = 1 # 除了首帧以外都要生成
                    if not mask_conditions[:, :, 0, :, :].any(): # 首帧是完全清晰的，后面全是noisy的
                        ...
                        timesteps = torch.cat([temp_ts, temp_ts.new_ones(mask_conditions_bs, seq_len - temp_ts.size(1)) * timesteps[:, None,]], dim = 1) # 第一帧对应的timesteps是0，后面是正常的。需要进行修正：condition的action和proprio的timesteps是0，其他的是正常的
                    else:
                ...
                if actions is not None:
                    actions = actions.to(accelerator.device, dtype=weight_dtype)
                if proprio is not None:
                    proprio = proprio.to(accelerator.device, dtype=weight_dtype)

                split_idx = None
                mask_for_split = batch.get('mask', None)
                if mask_for_split is not None:
                    m = mask_for_split[:, :, 0, 0, 0]
                    split_idx_list = []
                    ...
                    if len(split_idx_list) > 0 and len(set(split_idx_list)) == 1:
                        split_idx = split_idx_list[0]
                    elif len(split_idx_list) > 0:
                        split_idx = split_idx_list[0]
                ...
                # Append timesteps for extra tokens: [action_cond, action_target, proprio_cond, proprio_target]
                # action_cond and proprio_cond use timesteps=0, action_target and proprio_target use original timesteps
                if spatial_compression_ratio >= 16 and timesteps.dim() == 2:
                    bsz = timesteps.size(0)
                    orig_timesteps = timesteps[:, -1:]  # [B, 1] - the original timestep value
                    extra_ts_list = []
                    if actions_cond is not None:
                        num_action_cond = actions_cond.size(1)
                        extra_ts_list.append(timesteps.new_zeros(bsz, num_action_cond))
                    if actions_target is not None:
                        num_action_target = actions_target.size(1)
                        extra_ts_list.append(orig_timesteps.expand(-1, num_action_target))
                    if proprio_cond is not None:
                        num_proprio_cond = 1  # proprio embedding only takes [:,-1:]
                        extra_ts_list.append(timesteps.new_zeros(bsz, num_proprio_cond))
                    if proprio_target is not None:
                        num_proprio_target = 1  # proprio embedding only takes [:,-1:]
                        extra_ts_list.append(orig_timesteps.expand(-1, num_proprio_target))
                    if extra_ts_list:
                        extra_ts = torch.cat(extra_ts_list, dim=1)
                        timesteps = torch.cat([timesteps, extra_ts], dim=1)

                # Add noise to actions and proprio (similar to video noise injection)
                # cond: weak noise sigma=0.0001, target: same noise as video (sigmas)
                sigma_cond = 0.0000
                actions_cond_noisy, actions_target_noisy = None, None
                proprio_cond_noisy, proprio_target_noisy = None, None
                actions_noise, proprio_noise = None, None
                
                if actions_cond is not None:
                    noise_cond = torch.randn_like(actions_cond)
                    actions_cond_noisy = (1.0 - sigma_cond) * actions_cond + sigma_cond * noise_cond
                if actions_target is not None:
                    actions_noise = torch.randn_like(actions_target)
                    # sigmas shape: [B,1,1,1,1], need to broadcast to action shape [B, T, 7]
                    sigma_action = sigmas[:, 0, 0, 0, 0].unsqueeze(-1).unsqueeze(-1)  # [B, 1, 1]
                    actions_target_noisy = (1.0 - sigma_action) * actions_target + sigma_action * actions_noise
                
                if proprio_cond is not None:
                    noise_cond = torch.randn_like(proprio_cond)
                    proprio_cond_noisy = (1.0 - sigma_cond) * proprio_cond + sigma_cond * noise_cond
                if proprio_target is not None:
                    proprio_noise = torch.randn_like(proprio_target)
                    sigma_proprio = sigmas[:, 0, 0, 0, 0].unsqueeze(-1).unsqueeze(-1)  # [B, 1, 1]
                    proprio_target_noisy = (1.0 - sigma_proprio) * proprio_target + sigma_proprio * proprio_noise

                # Pack noisy versions for model input
                actions_in = (actions_cond_noisy, actions_target_noisy) if actions_target_noisy is not None else actions_cond_noisy
                proprio_in = (proprio_cond_noisy, proprio_target_noisy) if proprio_target_noisy is not None else proprio_cond_noisy
                
                # Ground truth for loss: noise - clean (same as video target)
                actions_gt = actions_noise - actions_target if actions_target is not None and actions_noise is not None else None
                # Model only predicts last time step for proprio (see proprio_target[:,-1:] in model)
                proprio_gt = proprio_noise[:,-1:] - proprio_target[:,-1:] if proprio_target is not None and proprio_noise is not None else None

                with torch.cuda.amp.autocast(dtype=weight_dtype), torch.cuda.device(device=accelerator.device):
                    noise_pred, action_pred, proprio_pred = transformer3d(
                        x=noisy_latents,
                ...
                loss = video_loss.clone()

                action_loss = torch.tensor(0.0, device=accelerator.device)
                proprio_loss = torch.tensor(0.0, device=accelerator.device)
                if action_pred is not None and actions_gt is not None and actions_gt.numel() > 0:
                    action_loss = F.mse_loss(action_pred.float(), actions_gt.float())
                    loss = loss + action_loss
                if proprio_pred is not None and proprio_gt is not None and proprio_gt.numel() > 0:
                    proprio_loss = F.mse_loss(proprio_pred.float(), proprio_gt.float())
                ...
                avg_video_loss = accelerator.gather(video_loss.repeat(args.train_batch_size)).mean()
                train_video_loss += avg_video_loss.item() / args.gradient_accumulation_steps
                
                # Gather action_loss and proprio_loss for logging
                if action_pred is not None and actions_gt is not None and actions_gt.numel() > 0:
                    avg_action_loss = accelerator.gather(action_loss.repeat(args.train_batch_size)).mean()
                    train_action_loss += avg_action_loss.item() / args.gradient_accumulation_steps
                    action_loss_seen = True
                if proprio_pred is not None and proprio_gt is not None and proprio_gt.numel() > 0:
                    avg_proprio_loss = accelerator.gather(proprio_loss.repeat(args.train_batch_size)).mean()
                    train_proprio_loss += avg_proprio_loss.item() / args.gradient_accumulation_steps
                    proprio_loss_seen = True

                # Backpropagate
                accelerator.backward(loss)
                if accelerator.sync_gradients:
                    # Sanity check: log embed gradients before clipping (every 100 steps)
                    if accelerator.is_main_process and global_step % 50 == 0:
                        _log_robot_embed_sanity(unwrap_model(transformer3d), global_step, writer)
            ...
            if accelerator.sync_gradients:
                ...
                # Prepare log dict with accumulated losses
                log_dict = {
                ...
                if action_loss_seen:
                    log_dict["action_loss"] = train_action_loss
                if proprio_loss_seen:
                    log_dict["proprio_loss"] = train_proprio_loss
                
                # Log to wandb/tensorboard via accelerator
                accelerator.log(log_dict, step=global_step)
                
                # Also log to TensorBoard directly for more control (on main process only)
                if accelerator.is_main_process:
                    writer.add_scalar("train/loss", train_loss, global_step)
                    writer.add_scalar("train/video_loss", train_video_loss, global_step)
                    if action_loss_seen:
                        writer.add_scalar("train/action_loss", train_action_loss, global_step)
                    if proprio_loss_seen:
                        writer.add_scalar("train/proprio_loss", train_proprio_loss, global_step)
                ...
                if global_step % args.checkpointing_steps == 0:
                    if args.use_deepspeed or args.use_fsdp or accelerator.is_main_process:
                        ...
                        if args.checkpoints_total_limit is not None:
                            ...
                            checkpoints = [c for c in checkpoints if len(c.split("-")) >= 2 and c.split("-")[1].isdigit()]
                        ...
                        if not args.save_state:
                            ...
                            from safetensors.torch import save_file
                            robot_sd = _get_robot_extra_state_dict(accelerator.unwrap_model(transformer3d))
                            save_file(
                                robot_sd,
                                os.path.join(args.output_dir, f"checkpoint-{global_step}-robot_extra_modules.safetensors"),
                                metadata={"format": "pt"},
                            )
                        else:
...
if __name__ == "__main__":
    # import debugpy
    # debugpy.listen(("127.0.0.1", 5666))  # 只监听本机，最安全
    # print("debugpy is listening on 127.0.0.1:5666")
