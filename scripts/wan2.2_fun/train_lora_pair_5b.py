...
def parse_args():
    ...
    parser.add_argument(
        "--use_pair_dataset",
        action="store_true",
        help="Whether to use MovieBenchFirstFramePairDataset for pair training.",
    )
    parser.add_argument(
        "--dataset_video_root",
        type=str,
        default=None,
        help="Root path for video files in pair dataset.",
    )
    parser.add_argument(
        "--weighting_scheme",
        type=str,
...
def main():
    ...
    # Get the dataset
    if args.use_pair_dataset:
        # Use MovieBenchFirstFramePairDataset for pair training
        train_dataset = MovieBenchFirstFramePairDataset(
            ann_path=args.train_data_meta,
            video_root=args.dataset_video_root,
    ...
    if args.enable_bucket:
        ...
        # Use dataset_video_root for pair dataset, otherwise use train_data_dir
        bucket_train_folder = args.dataset_video_root if args.use_pair_dataset and args.dataset_video_root else args.train_data_dir
        ...
        def collate_fn(examples):
            # For MovieBenchFirstFramePairDataset, examples is a list with single item
            if args.use_pair_dataset:
                # MovieBenchFirstFramePairDataset returns a list with one batch item
                examples = [examples] if not isinstance(examples, list) else examples
            ...
            for example in examples:
                if args.use_pair_dataset:
                    # MovieBenchFirstFramePairDataset returns torch tensor when enable_bucket=False
                    # When enable_bucket=True, it returns numpy array
                    if isinstance(example["pixel_values"], np.ndarray):
                        # To 0~1
                        pixel_values = torch.from_numpy(example["pixel_values"]).permute(0, 3, 1, 2).contiguous()
                        pixel_values = pixel_values / 255.
                    else:
                        # Already torch tensor (already normalized to -1~1)
                        pixel_values = example["pixel_values"]
                        new_examples["pixel_values"].append(pixel_values[:batch_video_length])
                        new_examples["text"].append(example["text"])
                        continue

                    # Apply resize and normalization for bucket mode (numpy input)
                    b, c, h, w = pixel_values.size()
                    closest_size_pair = list(map(lambda x: int(x), closest_size))
                    if closest_size_pair[0] / h > closest_size_pair[1] / w:
                        resize_size = closest_size_pair[0], int(w * closest_size_pair[0] / h)
                    else:
                        resize_size = int(h * closest_size_pair[1] / w), closest_size_pair[1]

                    transform = transforms.Compose([
                        transforms.Resize(resize_size, interpolation=transforms.InterpolationMode.BILINEAR),
                        transforms.CenterCrop(closest_size_pair),
                        transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5], inplace=True),
                    ])
                    new_examples["pixel_values"].append(transform(pixel_values)[:batch_video_length])
                    new_examples["text"].append(example["text"])
                elif args.fix_sample_size is not None:
                    # To 0~1
                    pixel_values = torch.from_numpy(example["pixel_values"]).permute(0, 3, 1, 2).contiguous()
                    pixel_values = pixel_values / 255.
        ...
        # DataLoaders creation:
        if args.use_pair_dataset:
            # For MovieBenchFirstFramePairDataset, create a simple collate_fn
            def pair_collate_fn(examples):
                new_examples = {
                    "pixel_values": [],
                    "text": [],
                }
                ...
                new_examples["pixel_values"] = torch.stack([p for p in new_examples["pixel_values"]])
                return new_examples
