...
def main():
    ...
    if str(args.dataset_type).lower() == "character_pair":
        if not isinstance(args.recaption_root, str) or not args.recaption_root:
            raise ValueError("--recaption_root is required when --dataset_type=character_pair")

        video_root = args.video_root
        if not isinstance(video_root, str) or not video_root:
            video_root = args.train_data_dir

        train_dataset = CharacterVideoEditingDataset(
...
if __name__ == "__main__":
    ...
    # debugpy.listen(("127.0.0.1", 5679))  # 只监听本机，最安全
