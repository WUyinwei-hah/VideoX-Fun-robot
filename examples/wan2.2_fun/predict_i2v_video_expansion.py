...
validation_video_start  = "/gemini/code/UNIFY_REFER_GEN/selected_noface_videos/023.mp4"
...
video_start_sec         = 0.0
video_end_sec           = 2.5   # None 表示使用视频总时长（秒）
prompt              = "No-Face sets down the basin of meat, turns his body to face the camera. 他用他黑色身体上的留着口水的嘴在说着什么."
negative_prompt     = ""
save_path           = "samples/wan2.2fun-noface_expansion"
# Other params
# 自动读取图片并根据其宽高比设置 480p 分辨率
if validation_video_start is not None and os.path.exists(validation_video_start):
    cap = cv2.VideoCapture(validation_video_start)
    ret, frame = cap.read()
    cap.release()
    if ret:
        ...
        print(f"Detected video frame size: {width}x{height}, aspect ratio: {aspect_ratio:.2f}")
        print(f"Set sample_size to: {sample_size}")
    elif validation_image_start is not None and os.path.exists(validation_image_start):
        ...
        sample_size = [480, 864]
        print(f"Warning: validation_image_start and validation_video_start not usable, using default sample_size: {sample_size}")
elif validation_image_start is not None and os.path.exists(validation_image_start):
    image = Image.open(validation_image_start)
    width, height = image.size
    aspect_ratio = width / height
...
with torch.no_grad():
    ...
    if validation_video_start is not None and os.path.exists(validation_video_start):
        ...
        if total_frames > 0:
            # 根据秒数和目标 fps 计算在目标时间轴上的帧索引
            target_fps = fps if fps is not None else 16
            if original_fps is None or original_fps <= 0:
                original_fps = target_fps

            start_time = max(0.0, float(video_start_sec))
            if video_end_sec is None:
                # 如果没指定结束秒数，就用视频总时长
                total_duration = total_frames / max(original_fps, 1e-6)
                end_time = total_duration
            else:
                end_time = max(start_time, float(video_end_sec))
            ...
            # 在时间 [start_time, end_time) 上以 target_fps 采样
            import numpy as _np
            num_target_frames = int(_np.floor((end_time - start_time) * target_fps))
            if num_target_frames <= 0:
                num_target_frames = int(total_frames)
                start_time = 0.0
                end_time = total_frames / max(original_fps, 1e-6)

            target_times = start_time + _np.arange(num_target_frames) / target_fps
            # 映射到原视频帧索引
            target_indices = []
        ...
        if len(frames) > 0:
            validation_image_start_for_latent = frames
        else:
            validation_image_start_for_latent = validation_image_start
    else:
        validation_image_start_for_latent = validation_image_start

    input_video, input_video_mask, clip_image = get_image_to_video_latent(validation_image_start_for_latent, validation_image_end, video_length=video_length, sample_size=sample_size)
...
def save_results():
    ...
    if video_length == 1:
        ...
        ref_video_path = os.path.join(save_path, prefix + "_ref.mp4")
        if 'input_video' in globals() or 'input_video' in locals():
            save_videos_grid(input_video, ref_video_path, fps=fps)
