...
# Video 2 video
# 1. 修改数据：原始视频|修改的视频，prompt是原始的视频+view suddenly change+新的视频
# 2. 修改mask
# 可选：修改index 
...
def parse_args():
    ...
    parser.add_argument(
        "--recaption_key",
        type=str,
        default="Recaption Prompt",
    )
    parser.add_argument(
        "--pairing_strategy",
        type=str,
        default="random",
        choices=("random", "closest_length"),
    )
    parser.add_argument(
        "--max_pair_candidates",
        type=int,
...
def main():
    ...
    if str(args.dataset_type).lower() == "character_pair":
        ...
        dataset_cls = ImageVideoDataset
        if isinstance(args.train_data_meta, str) and args.train_data_meta.endswith('.jsonl'):
            dataset_cls = ImageVideoEditingDataset
    ...
    if args.enable_bucket and hasattr(train_dataset, 'dataset'):
        aspect_ratio_sample_size = {key : [x / 512 * args.video_sample_size for x in ASPECT_RATIO_512[key]] for key in ASPECT_RATIO_512.keys()}
        batch_sampler_generator = torch.Generator().manual_seed(args.seed)
        # Use video_root for character_pair dataset, otherwise use train_data_dir
        bucket_train_folder = args.video_root if str(args.dataset_type).lower() == "character_pair" and args.video_root else args.train_data_dir
...
if __name__ == "__main__":
    ...
    # debugpy.listen(("127.0.0.1", 5675))  # 只监听本机，最安全
