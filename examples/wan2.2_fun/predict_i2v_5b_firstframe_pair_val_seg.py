...
def main():
    ...
    parser.add_argument("--src_frame_sampling", type=str, default="contiguous", choices=("contiguous", "uniform"))
    ...
    with torch.no_grad():
        ...
        for i, item in enumerate(items):
            # Use foreground video as condition if available, fallback to source
            foreground_path = item.get("foreground_source_abs", "")
            source_path = item.get("source_abs", "")
            if foreground_path and os.path.exists(foreground_path):
                condition_path = foreground_path
            elif source_path and os.path.exists(source_path):
                condition_path = source_path
            else:
                print(f"Missing source video: {source_path}, skipping...")
                continue

            # Build prompt: SEP + edited_video_prompt (following training paradigm)
            prompt = str(item.get("prompt") or "").strip()
            if not prompt:
                edt_prompt = str(item.get("edited_video_prompt", "")).strip()
                prompt = (SEP + " " + edt_prompt).strip()

            # Compute sample size from video (use source for AR, condition for frames)
            height, width = _compute_hw_from_video_keep_ar(
                source_path if source_path else condition_path,
